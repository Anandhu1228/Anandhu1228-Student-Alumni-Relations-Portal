...
training_data_path = os.path.join(script_dir, 'training_doc2vec_internship_data.json')
model_save_path = os.path.join(script_dir, 'doc2vec_internship_rec_model')
...
for item in data:
    ...
    text = ' '.join(experiences + interests + [location])
...
if os.path.exists(model_save_path):
    ...
    model = Doc2Vec(vector_size=100,  # Adjust as needed
                    window=2,  # Adjust as needed
                    min_count=1,  # Adjust as needed
                    workers=8,  # Increase number of workers for faster training
                    epochs=20)  # Keep epochs as desired
...
print('Doc2Vec Internship recommendation model trained and saved successfully.')
